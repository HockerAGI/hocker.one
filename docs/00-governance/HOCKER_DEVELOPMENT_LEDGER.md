...
|---|---|---|---|---|
| `HockerAGI/hocker.one` | public | Hocker One control plane; governance of 16 AGIs | Protected `main` advanced from PR #228 to verified merge `cd1f8ef1d148394955013252ac06b2add8c0f460` from PR #229. PR #229 reconciles Core Integration Ready vs Full Launch/GA and retires stale operational dependence on #209/#215 while preserving them as historical evidence; final head `9852ef3627e896cd6dcf502917847a517be20a71` had GitHub Actions CI `32002493287` = SUCCESS and Vercel Preview `dpl_5VyirBUYy7QzZWLwhrFvDAXjVpDG` = READY before merge. No runtime, DDL, provider, secret or AGI-permission mutation is part of #229. New PR #230 is open/mergeable/draft at exact head `052060abd79f97b4018a6764607cdfa27c881c01`; it adds a resumable Owner certification UI/control for the 16 canonical AGIs while preserving `allow_actions=false`, AAL2 endpoints, server-derived durable evidence, sequential execution and fail-closed canonical-ID checks. Exact-head CI `32004789564` = SUCCESS and PR evidence records CodeQL/code scanning SUCCESS, but exact-head Vercel status is FAILURE because the account is deployment-rate-limited; no exact-head Preview exists, and the PR has zero submitted reviews. PR #215 remains the governance Ledger PR. | Treat `cd1f8ef1...` as current mutable `main`. Do not promote #230 while draft/review gates remain open and exact-head Vercel Preview/build-log evidence is unavailable. Core mandatory blockers remain Owner AAL2 ceremony, 16/16 runtime/tool certification evidence and Supabase leaked-password protection according to the merged closure pack. Preserve GraphQL/RPC exception boundaries as contracts, not blanket security closure. Physical Node Agent, dedicated Railway fallback, Android release, Casino/Wallet/KYC and regulated actions remain separately gated. |
...
| `HockerAGI/nova.agi` | public | dedicated NOVA runtime | PR #32 is now merged to `main` at `db417f262dfcddcad8e82f6be977415d0b0f3e89`. Final head `5b7d97cf1a3302bcb5dacf9349db81ccc07e12e0` had CI #151 SUCCESS and adds durable startup/recovery/handoff documentation plus continuity-contract tests without runtime code, secrets or DDL changes. Dedicated live Railway revision, `/health/ready`, runtime logs/heartbeat and authenticated Hocker One→NOVA E2E remain unverified. Hocker One unified NOVA runtime remains primary. | Treat continuity documentation as merged authority, but preserve dedicated Railway runtime as `PENDING EVIDENCE` until exact live deployment + readiness + logs + authenticated E2E are observed. |
| `HockerAGI/hocker-node-agent` | public | authorized local executor | Repository remains unchanged on its default branch. Hocker One's mirror-side liveness logic is corrected to use `public.nodes.id,last_seen_at`, but this is not evidence that a physical Node Agent is currently live. | `PENDING EVIDENCE`: verify current node-agent main SHA, CI, allowlists, HMAC/non-root/sandbox posture and fresh physical heartbeat before asserting live readiness. |
...
| `HockerAGI/punto.g` | private | PUNTO·G separate platform | PR #4 merged documentation-only provider-neutral Canon `1.0-rc4` at `31accb4c1ebc965a037578431f5a017e3728df60`. The current design supersedes mandatory Supabase DB/Auth/Realtime/Storage assumptions for future PUNTO·G work: PostgreSQL is the portable persistence contract, Neon is the first DB provider selected for later activation without becoming a domain dependency, Better Auth is CURRENT behind a PUNTO-owned identity boundary, and S3-compatible object storage is required with Cloudflare R2 candidate/preferred. HOCKER One administration is constrained to a minimized audited Control Contract. The merge is documentation-only and created/configured no provider, secret, deployment, KYC or payment resource. | Preserve rc4 provider-neutral contracts in future implementation. Do not reinterpret historical Supabase-oriented Phase 2 evidence as current provider authority. No real Neon/R2/KYC/payment/production activation without independent provider, security and legal gates. Global 10-app classification remains a separate governance decision. |
...
- PR #229 final head `9852ef3627e896cd6dcf502917847a517be20a71` has exact Preview `dpl_5VyirBUYy7QzZWLwhrFvDAXjVpDG` = READY and Vercel commit status SUCCESS. An exact production deployment for merge commit `cd1f8ef1...` was not observed in the current deployment window, so production-deployment identity for this documentation-only merge is `PENDING EVIDENCE` rather than assumed.
- PR #230 exact head `052060abd79f97b4018a6764607cdfa27c881c01` has **no exact-head Preview**. GitHub combined status reports Vercel = FAILURE with provider target indicating deployment/build rate limit. Earlier branch previews such as `dpl_5qTa5Giqg5Z9ZeWVCUgPPiFBYg96` are READY only for earlier commit `00da1a8425ffa50e2ed2a3877177a464e10b8a9d` and do not satisfy the exact-head gate.
- Last fully observed production deployment before the docs-only #229 merge remains PR #228/main `dpl_4EEB5MTDvgGaEVNGx5nRwFHWTQUN`, commit `6b3b4f35820f4fb9c0906fa582dcd397d3169f88`, state READY. This is a deployment pointer, not current `main` code authority.
...
7. **Core closure documentation is now merged, but certification is not:** #229 closed its prior RED regression and merged the Core Integration Ready closure pack. #230 is the active 16-AGI runtime/tool certification candidate, with exact-head CI green but blocked by draft/review state and Vercel rate limiting.
8. **NOVA recovery documentation moved from open PR to `main`:** #32 is merged, but dedicated Railway live/E2E evidence remains absent.
9. **PUNTO·G architecture changed materially at documentation authority:** rc4 is provider-neutral and supersedes mandatory Supabase assumptions for future PUNTO work without changing any live provider state.
10. **Legacy NOVA ownership remains intentionally unresolved:** fail-closed reconciliation backlog remains; no current change authorizes automatic ownership assignment.
11. **Hocker Ads E-060 remains draft/provider-neutral; regulated/destructive surfaces remain fail-closed.**
...
| Supabase performance | #227 closed three unindexed canonical AGI FKs. | Preserve new indexes until workload evidence supports any tuning/removal. | IMPLEMENTED / OBSERVATION |
...
| Hocker One continuity | #229 is merged and now carries current Core Integration Ready/Full Launch separation. #230 implements resumable Owner certification for the remaining AGI runtime/tool evidence path. | Keep #230 draft/unmerged until exact-head Vercel Preview + logs, required review/Owner evidence and all branch-protection gates are satisfied. | CORE DOCS MERGED / CERTIFICATION BLOCKED |
| NOVA continuity | #32 merged durable recovery documentation, but dedicated NOVA live Railway evidence remains missing; Hocker One unified runtime is primary. | Preserve `PENDING EVIDENCE` for dedicated runtime until exact deployment/readiness/logs/E2E evidence exists. | DOCS MERGED / LIVE RUNTIME UNVERIFIED |
...
| PUNTO·G | PR #4 rc4 now establishes provider-neutral current architecture while global canonical inventory still treats PUNTO·G as separate from the 10 apps. | Preserve rc4 editable sources as PUNTO authority; classify platform globally without silently changing canonical app count. | RC4 MERGED / GLOBAL CLASSIFICATION PENDING |

## Main/merge audit

**Three material merges were observed since the previous Ledger cut; none was performed by this audit.**

- Hocker One PR #229 merged documentation/continuity closure at `cd1f8ef1d148394955013252ac06b2add8c0f460`. Final head `9852ef3627e896cd6dcf502917847a517be20a71` had CI `32002493287` = SUCCESS and Vercel Preview `dpl_5VyirBUYy7QzZWLwhrFvDAXjVpDG` = READY. Scope is documentation/tests only; no runtime/DDL/provider/secret/permission mutation.
- NOVA PR #32 merged at `db417f262dfcddcad8e82f6be977415d0b0f3e89`; final head `5b7d97cf1a3302bcb5dacf9349db81ccc07e12e0` had CI #151 SUCCESS. This promotes recovery/continuity contracts, not proof of a live Railway deployment.
- PUNTO·G PR #4 merged documentation-only rc4 provider-neutral architecture at `31accb4c1ebc965a037578431f5a017e3728df60`; it explicitly performs no infrastructure/provider/secret/deployment/KYC/payment mutation.
- Hocker One PR #230 is open/mergeable/draft at `052060abd79f97b4018a6764607cdfa27c881c01`. Exact-head CI `32004789564` = SUCCESS, but Vercel exact-head status is FAILURE due deployment rate limiting, no exact-head Preview exists, and zero submitted reviews were observed. **Do not merge.**
- No new default-branch commits were observed for `hocker.agi`, `hocker-node-agent`, `chido.casino`, `chido.lab`, `chido.games` or `hocker.ads` in the monitored interval.
- Casino/Wallet/KYC/NEXPA/Trackhok/financial/destructive/autonomy and other Owner-gated surfaces remain fail-closed unless separately authorized with required evidence.
...
### 2026-08-17 00:15 PDT — Core closure merged; NOVA continuity and PUNTO rc4 advanced; AGI certification blocked externally

- Detected Hocker One PR #229 transition from failed draft to merged documentation authority. Final head `9852ef3627e896cd6dcf502917847a517be20a71` had GitHub Actions CI `32002493287` = SUCCESS and Vercel Preview `dpl_5VyirBUYy7QzZWLwhrFvDAXjVpDG` = READY; merge commit is `cd1f8ef1d148394955013252ac06b2add8c0f460`. The merged scope changes closure/continuity documentation and tests only; it does not modify runtime, DDL, providers, secrets or AGI permissions.
- Detected new Hocker One PR #230 at exact head `052060abd79f97b4018a6764607cdfa27c881c01`. It adds resumable Owner/AAL2 certification UX for the 16 canonical AGIs, reusing existing runtime/tool eval endpoints and durable server-derived evidence while preserving `allow_actions=false` and fail-closed canonical-set checks. CI `32004789564` = SUCCESS; PR evidence records CodeQL/code-scanning SUCCESS. However the PR remains draft with zero reviews, and exact-head Vercel combined status is FAILURE due provider deployment rate limiting; no exact-head Preview exists. Earlier READY branch previews do not satisfy this gate. No merge was performed.
- Detected NOVA PR #32 merged at `db417f262dfcddcad8e82f6be977415d0b0f3e89`. Final head `5b7d97cf1a3302bcb5dacf9349db81ccc07e12e0` had CI #151 SUCCESS. This promotes durable recovery/continuity documentation and contract tests only; exact Railway deployment revision, readiness, runtime logs/heartbeat and authenticated Hocker One→NOVA E2E remain `PENDING EVIDENCE`.
- Detected PUNTO·G PR #4 merged documentation-only provider-neutral Canon `1.0-rc4` at `31accb4c1ebc965a037578431f5a017e3728df60`. Future PUNTO persistence is now specified through a portable PostgreSQL contract with Neon selected for later activation, Better Auth behind a PUNTO-owned identity boundary, and S3-compatible object storage with R2 candidate/preferred. No provider, secret, deployment, KYC, payment or infrastructure resource was created by the PR.
- Cross-repo default-branch scan found no new commits for `hocker.agi`, `hocker-node-agent`, `chido.casino`, `chido.lab`, `chido.games` or `hocker.ads` in the monitored interval.
- Direct Supabase recheck: production remains `ACTIVE_HEALTHY`, `main=FUNCTIONS_DEPLOYED`; current Security Advisor still reports GraphQL exposure WARNs, existing SECURITY DEFINER RPC WARNs and leaked-password protection disabled. No migration/RLS/grant regression was observed and no production mutation was performed by this audit.
- Documentation drift is partially self-correcting: #229, NOVA #32 and PUNTO #4 update their own editable continuity/architecture sources. Global DOC-00/DOC-05/DOC-07 repository/provider inventory drift remains open and requires owner-led canonical reconciliation before any derived PDF regeneration.
- Performed no merge, production DDL, grant/RLS/secret/provider mutation, regulated activation or Owner-gated action. Updated only this governance Ledger branch.
