...
last_verified_at: 2026-08-17T16:56:58-07:00
...
- PUNTO·G repository `main`: `a19f2c60b4533451066be196fdc7bfdef2f17a29`; this is **177 commits ahead** of the prior Ledger pointer `acca73397d28a542b31e2faae3c98ba7436198f5` and includes Phase 2D control-contract work plus Phase 3A account/onboarding foundations and their editable documentation/audit records.
- PUNTO·G GitHub `main` is currently **not protected**; no required status checks are enforced by branch protection at this cut.
- PUNTO·G Neon development/validation project: `PUNTO.G` / `frosty-mode-96257627`; default `main=br-little-art-au9zcb71` is `ready` and directly contains the Phase 2D control tables plus Phase 3A onboarding schema. Current child branches are `phase-3b-marketplace-20260817=br-billowing-feather-auofo6jb`, `qa-phase3a-concurrent-privacy=br-steep-violet-augu5zel`, `backup-pre-phase3a-20260817=br-lingering-morning-aunfy131`, `backup-pre-phase2d-20260817=br-dry-thunder-auj9gu3z` and `backup-pre-phase2b-20260817=br-soft-art-aubt3ald`. Production/customer activation remains separately gated.
...
**Current repository authority is materially newer:** `punto.g/main=a19f2c60b4533451066be196fdc7bfdef2f17a29`, **177 commits ahead** of the prior Ledger pointer `acca73397d28a542b31e2faae3c98ba7436198f5`. The delta includes Phase 2D HOCKER Control Contract code/migration/audit/ADR plus Phase 3A account/onboarding implementation, Better Auth/email boundaries, design-system work and corresponding editable Canon/PRD/data/UX/legal/runbook/roadmap sources. GitHub `main` contains `db/migrations/0007_hocker_control_contract.sql`, `0008_phase3a_account_onboarding.sql`, `0009_phase3a_onboarding_consent.sql` and `0010_phase3a_atomic_privacy_step.sql`.

Direct Neon `main` confirms `control_private.project_control_state`, `control_private.control_command_log` and `identity_private.account_onboarding` exist. The Phase 3A audit classifies the phase **TECHNICALLY CLOSED / ACTIVATION GATED**, records 28/28 targeted contract checks PASS, real Neon transaction/ACL verification, an empty dev-vs-main schema diff after promotion and zero test residue. It explicitly **does not claim a fresh full current-head GitHub Actions run, Next.js build or lint pass** because CI quota/local dependency constraints prevented that broader gate. Public signup, final legal docs, transactional email provider/sender, distributed rate limiting, production login/capability membership and controlled deployment smoke remain fail-closed.

Governance risk remains open: GitHub directly reports `punto.g/main` as **unprotected** with zero required status checks. The current Phase 3A integration record uses a documented integration-equivalent/fast-forward approach rather than a protected PR gate, and the latest main commit is unsigned. Do not treat targeted verification as a replacement for branch protection or full exact-head CI.

Direct Neon state now exposes six branches. The earlier `phase-2d-control-contract` child branch is no longer present, while default `main` contains the Phase 2D control objects. Ready child branches include `qa-phase3a-concurrent-privacy` and new `phase-3b-marketplace-20260817`; the latter is evidence that Phase 3B preparation has begun at provider level only. Phase 3B implementation/approval remains **PENDING EVIDENCE** until repository code/contracts/tests establish the exact delta.
...
- PR #231: active Ledger reconciliation PR. Exact head `98d25576ba9511684c3675ba633401aa3432de96` has Vercel Preview `dpl_uMcRu4UsuvoA6pV4ibuKiEsGxgBz` = READY; GitHub returns no workflow for this Markdown-only head. It remains open, non-draft and mergeable; this update creates a new exact head that must be gated independently.
...
- This 16:56 evidence reconciliation creates a new Ledger exact head that must be gated independently; no previous Preview state is inherited.
