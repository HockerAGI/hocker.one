...
La activación vigente es **evidence-bound** y **tamper-evident en la capa de binding de evidencia de aplicación**: el runtime enlaza la aprobación a acción, recurso, proyecto, candidate SHA, environment, request hash, trace/nonce, expiración y consumo único, y rechaza evidencia que no corresponde al request aprobado o una aprobación reutilizada. Esta propiedad no debe confundirse con una base inmutable ni con resistencia criptográfica independiente frente a modificación privilegiada; `service_role` conserva los privilegios internos necesarios sobre la persistencia. La identidad owner continúa siendo key-based en esta etapa y no equivale a una prueba nominal de persona, sesión MFA o identidad humana fuerte.

La siguiente evolución funcional es ampliar generación/lectura de manifest y coverage y conectar adaptadores concretos que publiquen checkpoints normalizados sin importar conversaciones completas. Cualquier escritura externa continúa sujeta a Owner Gate, trazabilidad y evidencia. Si en el futuro se requiere resistencia independiente frente a modificación privilegiada, deberá implementarse y probarse un mecanismo append-only, anclaje externo o verificación de integridad independiente antes de declarar esa propiedad.
