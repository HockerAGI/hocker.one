...
**Files:**
- Existing PR source files are the only intended functional files: `supabase/migrations/<work-session-migration>.sql`, `src/.../store.ts` where the current PR changed it, and the relevant Work Session regression test files.
...
Use GitHub changed-files and patch evidence so no unrelated file is carried forward.
...
The PR base must be the reconciled current `main` lineage, not the stale base currently recorded on #371.
