...
async function executeLocalCloud(command: string, payload: JsonRecord): Promise<CloudExecResult> {
  ...
  if (READONLY_COMMANDS.has(cmd)) {
    ...
    if (cmd === "read_file_head") {
      ...
      return {
        command: cmd,
        ok: true,
        data: { path: file, lines, totalLines: raw.split(/\r?\n/).length },
      };
    }
  }
  ...
  switch (cmd) {
    ...
    case "fs.write": {
      ...
      return {
        command: cmd,
        ok: true,
        data: { path: file, bytes: Buffer.byteLength(content, "utf8") },
      };
    }
...
export async function processCloudQueue(
  ...
  let query = sb
    .from("commands")
    .select("id,project_id,node_id,command,payload,signature,status,needs_approval,created_at")
