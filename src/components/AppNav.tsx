    ...
    <div className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/80 backdrop-blur-xl">
      <div className="mx-auto max-w-7xl px-4 py-3">
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between gap-3">
              ...
              {ITEMS.map((it) => {
                ...
                return (
                  <Link
                    ...
                    href={it.href}
                    className={
                      "group inline-flex items-center gap-2 rounded-2xl px-3 py-2 text-sm font-semibold transition-all " +
                      (isActive
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="hidden md:inline-flex items-center rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
                title="Volver"
              >
                Volver
              </button>
              ...
              <form action="/signout" method="post" className="hidden md:block">
                <button
                  type="submit"
                  className="inline-flex items-center rounded-2xl bg-slate-950 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800"
                  title="Cerrar sesión"
                >
                  Salir
                </button>
              ...
              <button
                type="button"
                onClick={() => setOpen((v) => !v)}
                className="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white p-2 hover:bg-slate-50 lg:hidden"
          ...
          {open && (
            <div className="grid gap-2 rounded-3xl border border-slate-200 bg-white p-3 shadow-lg shadow-slate-900/5 lg:hidden">
              ...
              <div className="flex items-center justify-between gap-2">
                <button
                  type="button"
                  onClick={() => window.history.back()}
                  className="w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50"
                >
                  Volver
                </button>
                <form action="/signout" method="post" className="w-full">
                  <button
                    type="submit"
                    className="w-full rounded-2xl bg-slate-950 px-3 py-2 text-sm font-semibold text-white hover:bg-slate-800"
                  >
                    Salir
                  </button>
                </form>
