    ...
    return (
      <div className="flex items-center justify-between px-4 pb-2">
        {ITEMS.map((it) => {
          ...
          const Icon = it.icon;
          return (
            <Link key={it.href} href={it.href} className={`relative flex flex-col items-center gap-1 rounded-2xl p-3 transition-all active:scale-95 ${isActive ? "text-sky-400" : "text-slate-500 hover:text-slate-300"}`}>
              {isActive && <div className="absolute -top-2 h-1 w-6 rounded-full bg-sky-400 shadow-[0_0_10px_rgba(14,165,233,0.8)]" />}
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto p-6 custom-scrollbar">
      <div className="mb-10 transition-transform hover:scale-105">
        <BrandMark showWordmark hero={false} className="scale-110 origin-left" />
      </div>
        ...
        <section>
          ...
          <div className="space-y-1.5">
            {ITEMS.map((it) => {
              ...
              const Icon = it.icon;
              return (
                <Link key={it.href} href={it.href} className={`group relative flex items-center gap-3 rounded-2xl px-4 py-3.5 text-[13px] font-bold transition-all active:scale-95 ${isActive ? "bg-sky-500/10 text-sky-400 shadow-[inset_3px_0_0_0_#0ea5ff]" : "text-slate-400 hover:bg-white/5 hover:text-slate-200"}`}>
                  <Icon className={`h-5 w-5 transition-colors ${isActive ? "text-sky-400" : "text-slate-500 group-hover:text-slate-300"}`} />
                  {it.label}
                  {isActive && <div className="absolute right-4 h-1.5 w-1.5 rounded-full bg-sky-400 shadow-[0_0_8px_rgba(14,165,233,0.8)] animate-pulse" />}
                </Link>
              );
            })}
          </div>
      ...
      <div className="mt-8 flex flex-col gap-3 border-t border-white/5 pt-6">
         <button onClick={() => router.back()} className="flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-black uppercase tracking-widest text-slate-300 transition-all hover:bg-white/10 active:scale-95">
         ...
         <form action="/signout" method="post">
           <button type="submit" className="w-full rounded-2xl bg-white px-4 py-3 text-xs font-black uppercase tracking-widest text-slate-950 transition-all hover:bg-slate-200 active:scale-95 shadow-lg">
             Desconectar Matriz
           </button>
         </form>
      </div>
