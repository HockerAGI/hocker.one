  ...
  return (
    <div className="relative overflow-hidden rounded-[26px] border border-white/10 bg-slate-950/45 p-4">
      ...
      <div className="mt-4 h-2 overflow-hidden rounded-full bg-white/8">
        <div className={`h-full rounded-full bg-gradient-to-r ${glow} transition-all duration-700`} style={{ width: `${width}%` }} />
      </div>
    </div>
  );
}
      ...
      <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h2 className="mt-4 text-2xl font-black leading-none tracking-[-0.05em] text-white sm:text-4xl">
            Evidencia almacenada en Supabase.
          </h2>

          <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-300">
            {summary.message} Estos conteos representan registros guardados; no prueban por sí solos que una AGI esté ejecutándose ahora.
          </p>
        </div>

        <Link href="/live" className="inline-flex items-center justify-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-5 py-3 text-[10px] font-black uppercase tracking-[0.22em] text-cyan-100 transition hover:bg-cyan-300/15">
          <Brain className="h-4 w-4" />
          Ver señales verificables
        </Link>
      </div>

      <div className="relative mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        <Metric label="Memorias habilitadas" value={summary.counts.active_memory} tone="emerald" />
        <Metric label="Aprendizajes aprobados" value={summary.counts.approved_learning} tone="cyan" />
        <Metric label="Actualizaciones registradas" value={summary.counts.active_agi_updates} tone="violet" />
        <Metric label="Patrones de error" value={summary.counts.prevented_errors} tone="amber" />
        <Metric label="Repeticiones registradas" value={summary.counts.repeated_seen} tone="emerald" />
      </div>
      ...
      <div className="relative mt-4 grid gap-3 lg:grid-cols-[1fr_0.8fr]">
        <div className="rounded-[30px] border border-white/10 bg-slate-950/45 p-5">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.24em] text-slate-500">Última memoria registrada</p>
          </div>
          ...
          <p className="mt-3 text-sm leading-7 text-slate-300">
            {latest
              ? `${humanAgiName(latest.source_agi_id)} figura como origen del registro. Destinos declarados: ${targets}. Veces consultado: ${latest.times_seen}.`
              : "Cuando se apruebe una memoria, aparecerá aquí como registro persistido."}
          </p>
        </div>
        ...
        <div className="rounded-[30px] border border-white/10 bg-slate-950/45 p-5">
          <div className="flex items-center gap-3">
            ...
            <p className="text-[10px] font-black uppercase tracking-[0.24em] text-slate-500">Flujo documentado</p>
          </div>
          ...
          <div className="mt-5 flex items-center gap-2 text-xs font-bold text-slate-400">
            ...
            Flujo conceptual; el estado de cada worker se consulta en AGIs.
          </div>
        </div>
      </div>
    </section>
