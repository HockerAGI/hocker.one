...
export default function AgiCard({ agi, featured = false }: { agi: AgiRegistryItem; featured?: boolean }) {
  const visualSrc = agi.iconSrc ?? agi.logoSrc;

  return (
    <article id={agi.key} className={["hko-module-card hko-card-tight hko-fade-up", featured ? "lg:col-span-2" : ""].join(" ")}>
      <div className="flex items-start gap-4">
        <ModuleLogoFrame title={agi.title} src={visualSrc} kind={agi.key === "nova" ? "nova" : "agi"} accent={agi.accent} size={featured ? "lg" : "md"} />
        <div className="min-w-0 flex-1">
          <h3 className="text-xl font-black tracking-tight text-white">{agi.title}</h3>
          <p className="mt-1 text-[15px] leading-relaxed text-slate-300">{agi.subtitle}</p>
          <div className="mt-3"><StatusBadge status={agi.status} compact /></div>
          <p className="mt-3 text-sm leading-relaxed text-slate-400">{agi.note}</p>
        </div>
      </div>

      <div className="mt-5 flex flex-wrap gap-2">
        <Link href={agi.href} className="hko-action-primary">Abrir módulo</Link>
