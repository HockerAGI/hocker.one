...
export default function AppCard({ app, featured = false }: { app: AppRegistryItem; featured?: boolean }) {
  const visualSrc = app.iconSrc ?? app.logoSrc;

  return (
    <article id={app.key} className={["hko-module-card hko-card-tight hko-fade-up", featured ? "lg:col-span-2" : ""].join(" ")}>
      <div className="flex items-start gap-4">
        <ModuleLogoFrame title={app.title} src={visualSrc} kind="app" accent={app.accent} size={featured ? "lg" : "md"} />
        <div className="min-w-0 flex-1">
          <h3 className="text-xl font-black tracking-tight text-white">{app.title}</h3>
          <p className="mt-1 text-[15px] leading-relaxed text-slate-300">{app.subtitle}</p>
          <div className="mt-3"><StatusBadge status={app.status} compact /></div>
          <p className="mt-3 text-sm leading-relaxed text-slate-400">{app.note}</p>
        </div>
      </div>

      <div className="mt-5 flex flex-wrap gap-2">
        <Link href={app.href} className="hko-action-primary">Abrir</Link>
