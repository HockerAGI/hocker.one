    ...
    if (call.provider === "agi" && call.tool === "delegate") {
      ...
      try {
        const delegated = await createServerlessAgiTask({
          ...
          context: { ...(asRecord(call.args.context)), parent_run_id: parentRunId, source_agi_id: options.source_agi_id ?? "nova", target_agi_ids: Array.isArray(call.args.target_agi_ids) ? call.args.target_agi_ids.slice(0, 12) : [target], delegation_depth: depth + 1, delegation_fanout: fanout + 1 },
