...
-- Incluye RPC atómico "supply_create_order"
-- Compatible con llamadas desde:
-- - usuarios authenticated (operator/admin/owner)
-- - backend service_role
-- ============================================

begin;

-- Asegura helper (por si aplican migraciones fuera de orden)
...
-- RLS (solo si quieres abrir lectura a miembros)
alter table public.supply_products enable row level security;

do $$
begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='supply_products' and policyname='hocker_supply_products_select_member') then
    execute $sql$
      create policy hocker_supply_products_select_member
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='supply_products' and policyname='hocker_supply_products_insert_admin') then
    execute $sql$
      create policy hocker_supply_products_insert_admin
      on public.supply_products
      for insert
      to authenticated
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='supply_products' and policyname='hocker_supply_products_update_admin') then
    execute $sql$
      create policy hocker_supply_products_update_admin
      on public.supply_products
      for update
      to authenticated
...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='supply_orders' and policyname='hocker_supply_orders_select_member') then
    execute $sql$
      create policy hocker_supply_orders_select_member
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='supply_orders' and policyname='hocker_supply_orders_insert_operator') then
    execute $sql$
      create policy hocker_supply_orders_insert_operator
      on public.supply_orders
      for insert
      to authenticated
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='supply_orders' and policyname='hocker_supply_orders_update_operator') then
    execute $sql$
      create policy hocker_supply_orders_update_operator
      on public.supply_orders
      for update
      to authenticated
...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='supply_order_items' and policyname='hocker_supply_order_items_select_member') then
    execute $sql$
      create policy hocker_supply_order_items_select_member
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='supply_order_items' and policyname='hocker_supply_order_items_insert_operator') then
    execute $sql$
      create policy hocker_supply_order_items_insert_operator
      on public.supply_order_items
      for insert
      to authenticated
      with check (public.is_project_operator(project_id));
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='supply_order_items' and policyname='hocker_supply_order_items_update_operator') then
    execute $sql$
      create policy hocker_supply_order_items_update_operator
      on public.supply_order_items
      for update
      to authenticated
      using (public.is_project_operator(project_id))
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='supply_order_items' and policyname='hocker_supply_order_items_delete_operator') then
    execute $sql$
      create policy hocker_supply_order_items_delete_operator
      on public.supply_order_items
      for delete
      to authenticated
      using (public.is_project_operator(project_id));
...
-- - Permite authenticated operators
-- - Permite backend service_role (sin auth.uid)
-- =========================
...
begin
  -- Permisos:
  -- 1) service_role => OK (backend)
  -- 2) authenticated operator => OK
  ...
  loop
    ...
    if v_unit is null then
      -- si hay producto, usa precio del producto
