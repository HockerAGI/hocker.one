...
test("live NOVA wording cannot fail certification solely because of lexical evidence diagnostics", async () => {
  ...
  const lexicalDiagnostic = rubric.scoreEvidence(live);
  assert.equal(typeof lexicalDiagnostic.passed, "boolean");
  assert.ok(Array.isArray(lexicalDiagnostic.reasons));

  assert.match(runner, /const lexicalDiagnostic = scoreEvidence\(completion\.text\)/);
  assert.match(runner, /const semanticGrade = await gradeEvidenceSemantically/);
  assert.match(runner, /if \(!semanticGrade\.passed\) reasons\.push\("evidence_semantic_failure"\)/);
  assert.doesNotMatch(runner, /reasons\.push\(\.\.\.lexicalDiagnostic\.reasons\)/);
});
