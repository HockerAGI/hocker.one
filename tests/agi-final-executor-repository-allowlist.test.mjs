...
test("final GitHub executor covers all six HOCKER repositories", async () => {
  const source = await read("src/lib/agi-action-execution.ts");

  assert.match(source, /HockerAGI\/hocker\.one,HockerAGI\/nova\.agi,HockerAGI\/hocker-node-agent,HockerAGI\/chido\.casino,HockerAGI\/hocker\.agi,HockerAGI\/hocker\.ads/);
