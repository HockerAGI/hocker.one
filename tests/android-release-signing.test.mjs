...
test("Android release build requires external signing secrets", async () => {
...
test("release workflow never stores the keystore in the repository", async () => {
  const workflow = await read(".github/workflows/android-release.yml");

  assert.match(workflow, /\$RUNNER_TEMP\/hocker-one-release\.jks/);
  assert.match(workflow, /chmod 600/);
  assert.match(workflow, /Remove temporary keystore/);
