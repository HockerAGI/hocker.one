...
test("live summary does not claim the dedicated NOVA runtime is live before certification", async () => {
  const source = await source("src/lib/hocker-live-summary.ts");
  assert.match(source, /runtime dedicado nova\.agi permanece sin certificación live/i);
  assert.doesNotMatch(source, /NOVA está viva y responde desde Railway/i);
});
